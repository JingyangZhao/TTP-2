...
{	
	...
	int FFF=1, opt=1, f_check=0, iter=1;
	
	//////////////////17 instances of even n/2
	//printf("even n/2:\n");
	for(int k=1; k<=8; k++) {srand(0);
		int n=40-4*(k-1), ite=iter; char *s = "Galaxy"; char *buf = new char[strlen(s) + sizeof(n) + 1]; sprintf(buf, "%s%d.txt", s, n);
		...
		int min=INT_MAX, target=0, temp_min; clock_t init_time; init_time=clock();
		while( ite-- ) {
			get_schedule(target, u, D, n, FFF, temp_min, day, f_check , opt); FFF = -FFF;
			if(min>temp_min) min=temp_min;
		} 
		printf("%d\n", min);
		for(int i=0; i<=n; i++) vector<int>().swap(day[i]),vector<int>().swap(D[i]); vector<vector<int> >().swap(day),vector<vector<int> >().swap(D); delete [] buf;
	}
	for(int k=1; k<=5; k++) {srand(0);
		int n=32-4*(k-1), ite=iter; char *s = "NFL"; char *buf = new char[strlen(s) + sizeof(n) + 1]; sprintf(buf, "%s%d.txt", s, n);
		FILE *fp=fopen(buf,"r"); vector<vector<int> > D(n+1,vector<int>(n+1)); read_D(fp, D, n); vector<vector<int> > day(n+1, vector<int>(2*n-2+1)); 		
		int lb=LB(D,n); pair<int, int> u[n/2+1]; for (int v = 1, f=1; v <= n; v++) if(mate[v]!=-1) u[f]=make_pair(v,mate[v]), mate[mate[v]]=-1, f++;
		int min=INT_MAX, target=0, temp_min; clock_t init_time; init_time=clock();
		while( ite-- ) {
	...
	for(int k=1; k<=2; k++){srand(0);
		int n=16-4*(k-1), ite=iter; char *s = "NL"; char *buf = new char[strlen(s) + sizeof(n) + 1]; sprintf(buf, "%s%d.txt", s, n);
		FILE *fp=fopen(buf,"r"); vector<vector<int> > D(n+1,vector<int>(n+1)); read_D(fp, D, n); vector<vector<int> > day(n+1, vector<int>(2*n-2+1)); 
		int lb=LB(D,n); pair<int, int> u[n/2+1]; for (int v = 1, f=1; v <= n; v++) if(mate[v]!=-1) u[f]=make_pair(v,mate[v]), mate[mate[v]]=-1, f++;
		int min=INT_MAX, target=0, temp_min; clock_t init_time; init_time=clock();
		while( ite-- ) {
	...
	for(int k=1; k<=1; k++){srand(0);
		int n=12-4*(k-1), ite=iter; char *s = "Super"; char *buf = new char[strlen(s) + sizeof(n) + 1]; sprintf(buf, "%s%d.txt", s, n);
		FILE *fp=fopen(buf,"r"); vector<vector<int> > D(n+1,vector<int>(n+1)); read_D(fp, D, n); vector<vector<int> > day(n+1, vector<int>(2*n-2+1)); 
		int lb=LB(D,n); pair<int, int> u[n/2+1]; for (int v = 1, f=1; v <= n; v++) if(mate[v]!=-1) u[f]=make_pair(v,mate[v]), mate[mate[v]]=-1, f++;
		int min=INT_MAX, target=0, temp_min; clock_t init_time; init_time=clock();
		while( ite-- ) {
	...
	for(int k=1; k<=1; k++){srand(0);
		int n=24-4*(k-1), ite=iter; char *s = "Brazil"; char *buf = new char[strlen(s) + sizeof(n) + 1]; sprintf(buf, "%s%d.txt", s, n);
		FILE *fp=fopen(buf,"r"); vector<vector<int> > D(n+1,vector<int>(n+1)); read_D(fp, D, n); vector<vector<int> > day(n+1, vector<int>(2*n-2+1)); 
		int lb=LB(D,n); pair<int, int> u[n/2+1]; for (int v = 1, f=1; v <= n; v++) if(mate[v]!=-1) u[f]=make_pair(v,mate[v]), mate[mate[v]]=-1, f++;
		int min=INT_MAX, target=0, temp_min; clock_t init_time; init_time=clock();
		while( ite-- ) {
	...
	//printf("odd n/2:\n");
	for(int k=1; k<=8; k++) {srand(0);
		int n=38-4*(k-1), ite=iter; char *s = "Galaxy"; char *buf = new char[strlen(s) + sizeof(n) + 1]; sprintf(buf, "%s%d.txt", s, n);
		FILE *fp=fopen(buf,"r"); vector<vector<int> > D(n+1,vector<int>(n+1)); read_D(fp, D, n); vector<vector<int> > day(n+1, vector<int>(2*n-2+1)); 		
		int lb=LB(D,n); pair<int, int> u[n/2+1]; for (int v = 1, f=1; v <= n; v++) if(mate[v]!=-1) u[f]=make_pair(v,mate[v]), mate[mate[v]]=-1, f++;	
		int min=INT_MAX, target=0, temp_min; clock_t init_time; init_time=clock();
		while( ite-- ) {
	...
	for(int k=1; k<=4; k++) {srand(0);
		int n=30-4*(k-1), ite=iter; char *s = "NFL"; char *buf = new char[strlen(s) + sizeof(n) + 1]; sprintf(buf, "%s%d.txt", s, n);
		FILE *fp=fopen(buf,"r"); vector<vector<int> > D(n+1,vector<int>(n+1)); read_D(fp, D, n); vector<vector<int> > day(n+1, vector<int>(2*n-2+1)); 		
		int lb=LB(D,n); pair<int, int> u[n/2+1]; for (int v = 1, f=1; v <= n; v++) if(mate[v]!=-1) u[f]=make_pair(v,mate[v]), mate[mate[v]]=-1, f++;
		int min=INT_MAX, target=0, temp_min; clock_t init_time; init_time=clock();
		while( ite-- ) {
	...
	for(int k=1; k<=2; k++){srand(0);
		int n=14-4*(k-1), ite=iter; char *s = "NL"; char *buf = new char[strlen(s) + sizeof(n) + 1]; sprintf(buf, "%s%d.txt", s, n);
		FILE *fp=fopen(buf,"r"); vector<vector<int> > D(n+1,vector<int>(n+1)); read_D(fp, D, n); vector<vector<int> > day(n+1, vector<int>(2*n-2+1)); 
		int lb=LB(D,n); pair<int, int> u[n/2+1]; for (int v = 1, f=1; v <= n; v++) if(mate[v]!=-1) u[f]=make_pair(v,mate[v]), mate[mate[v]]=-1, f++;
		int min=INT_MAX, target=0, temp_min; clock_t init_time; init_time=clock();
		while( ite-- ) {
	...
	for(int k=1; k<=2; k++){srand(0);
		int n=14-4*(k-1), ite=iter; char *s = "Super"; char *buf = new char[strlen(s) + sizeof(n) + 1]; sprintf(buf, "%s%d.txt", s, n);
		FILE *fp=fopen(buf,"r"); vector<vector<int> > D(n+1,vector<int>(n+1)); read_D(fp, D, n); vector<vector<int> > day(n+1, vector<int>(2*n-2+1)); 
		int lb=LB(D,n); pair<int, int> u[n/2+1]; for (int v = 1, f=1; v <= n; v++) if(mate[v]!=-1) u[f]=make_pair(v,mate[v]), mate[mate[v]]=-1, f++;
		int min=INT_MAX, target=0, temp_min; clock_t init_time; init_time=clock();
		while( ite-- ) {
